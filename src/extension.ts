...
import type { CloudUserInfo, AuthState } from "@roo-code-oss/types"
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("roo-cline-oss.plusButtonClicked")
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	if (process.env.NODE_ENV === "development") {
		const watchPaths = [
			...
			{ path: path.join(context.extensionPath, "node_modules/@roo-code-oss/cloud"), pattern: "**/*" },
		]
